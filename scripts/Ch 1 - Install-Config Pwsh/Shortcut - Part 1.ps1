...
#    NB: You create PowerSHell 7 profiles in a later step
#    this step creaees and ISE and a console profile from the same github download
...
$ISEProfileFile = $Profile.CurrentUserCurrentHost
New-Item $ISEProfileFile -Force -WarningAction SilentlyContinue |
   Out-Null
(Invoke-WebRequest -Uri $URI -UseBasicParsing).Content | 
  Out-File -FilePath  $ISEProfileFile
